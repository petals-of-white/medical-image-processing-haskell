...
isHomogeneous img = Foldl.fold Foldl.std (Prelude.map fromIntegral $ VS.toList $ manifestVector img) <= 7
...
saveBMP :: Storable a => FilePath -> (a -> RGBPixel) -> Manifest a -> IO ()
saveBMP path f img = 
    writeBitmap path $ toJuicyRGB $ I.map f img
